...
public class FindFlightsCommand(ApplicationDbContext context) : ICommandHandler<FindFlightRequest, List<FindFlightResponse>>
{
    public async Task<Result<List<FindFlightResponse>>> Handle(FindFlightRequest request, CancellationToken cancellationToken)
    {
        ...
        var departureDate = new DateOnly(request.DepartureDate.Year, request.DepartureDate.Month, request.DepartureDate.Day);

        var flights = await context.Flights.AsNoTracking().Include(a => a.OriginAirport).Include(a => a.DestinationAirport).Where(a => new DateTime(departureDate, startTime) <= a.DepartureDateTime && new DateTime(departureDate, endTime) >= a.DepartureDateTime && a.OriginAirportId == request.OriginAirportId && a.DestinationAirportId == request.DestinationAirportId).Select(a => new FindFlightResponse
        {
            Id = a.Id,
